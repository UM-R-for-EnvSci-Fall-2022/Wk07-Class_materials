...
There are two last options, when we want to include fixed lines (e.g. for a reference level). for this we use `geom_vline()`
...
Finally the `geom_abline()` which allow us to create a line between two points (hence the A B Line), or to provide an intercept and a slope to plot a line. Very usefult to set a 1:1 line for something like a QQ-Plot
